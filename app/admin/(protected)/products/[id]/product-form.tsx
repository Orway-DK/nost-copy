// app/admin/(protected)/products/[id]/product-form.tsx
"use client";
...
import { IoImageOutline, IoCubeOutline, IoConstructOutline, IoLanguageOutline, IoSave } from "react-icons/io5";
...
export default function ProductForm({ isNew, categories, initialProduct, initialVariants, initialLocalizations }: Props) {
    ...
    const [form, setForm] = useState({
        ...
        media_base_path: initialProduct?.media_base_path || "/public/media/products",
    ...
    const handleSave = async (e?: React.FormEvent) => {
        ...
        if (!form.name || !form.category_slug) return alert("Ad ve Kategori zorunlu.");
    ...
    return (
        <div className="grid gap-6 pb-20">
            ...
            <div className="border-b" style={{ borderColor: "var(--admin-card-border)" }}>
                <nav className="flex space-x-6">
                    {[
                        ...
                        { id: "variants", label: "Varyasyonlar", icon: IoConstructOutline, disabled: isNew },
                        { id: "localizations", label: "Çeviriler", icon: IoLanguageOutline, disabled: isNew },
                        { id: "media", label: "Medya", icon: IoImageOutline, disabled: isNew },
                    ].map((tab: any) => (
                        <button
                            key={tab.id}
                            onClick={() => !tab.disabled && setActiveTab(tab.id)}
                            disabled={tab.disabled}
                            className={`pb-3 flex items-center gap-2 font-medium transition-colors border-b-2 ${activeTab === tab.id
            ...
            {/* --- GENEL SEKME --- */}
            {activeTab === "general" && (
                <form onSubmit={handleSave} className="card-admin grid gap-8 animate-in fade-in">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div>
                            <div className="p-4 rounded-xl border" style={{ backgroundColor: "var(--admin-input-bg)", borderColor: "var(--admin-card-border)" }}>
                                <label className="admin-label mb-2 block">Kapak Görseli</label>
                                <div className="relative aspect-square w-full bg-[var(--admin-card)] rounded-lg overflow-hidden border flex items-center justify-center group"
                                ...
                                <div className="mt-2 text-xs opacity-50 truncate" title={form.main_image_url}>
                                    {form.main_image_url ? form.main_image_url.split('/').pop() : "Seçili değil"}
                                </div>
                            </div>
                        </div>
                    </div>
