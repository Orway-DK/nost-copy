...
export async function upsertCategoryAction(data: any) {
  try {
    ...
    if (
      data.id &&
      data.parent_id &&
      Number(data.id) === Number(data.parent_id)
    ) {
...
export async function deleteCategoryAction(id: number) {
  try {
    ...
    if (fetchError)
      throw new Error("Kategori bulunamadı: " + fetchError.message);
    ...
    if (productsError) {
      console.warn(
        "Ürünler güncellenemedi (category_slug NULL yapılamadı):",
    ...
    return {
      success: true,
      message:
        "Kategori silindi. Bağlı ürünlerin kategorisi NULL yapıldı, alt kategoriler yetim kaldı.",
    };
  } catch (error: any) {
    return { success: false, message: error.message };
  }
...
export async function getCategoryDetailsAction(id: number) {
  try {
    ...
    if (subcategoriesError)
      throw new Error(
        "Alt kategoriler getirilemedi: " + subcategoriesError.message
      );
...
export async function deleteCategoryWithOptionsAction(
  ...
  try {
    ...
    if (mapDeleteError) {
      console.warn(
        "product_category_map tablosundan silinemedi:",
        mapDeleteError.message
      );
    }
    ...
    if (subError)
      throw new Error("Alt kategoriler getirilemedi: " + subError.message);
    ...
    for (const subcat of subcategories || []) {
      ...
      if (subMapDeleteError) {
        console.warn(
          `Alt kategori ${subcat.id} product_category_map silinemedi:`,
          subMapDeleteError.message
        );
      }
      ...
      if (options.deleteProducts) {
        ...
        if (deleteProductsError) {
          console.warn(
            `Alt kategori ${subcat.id} ürünleri silinemedi:`,
        ...
        if (updateProductsError) {
          console.warn(
            `Alt kategori ${subcat.id} ürünleri güncellenemedi:`,
      ...
      if (deleteSubError) {
        console.warn(
          `Alt kategori ${subcat.id} silinemedi:`,
    ...
    if (options.deleteProducts) {
      ...
      if (deleteProductsError) {
        console.warn(
          "Ana kategori ürünleri silinemedi:",
          deleteProductsError.message
        );
      }
    } else {
      ...
      if (updateProductsError) {
        console.warn(
          "Ana kategori ürünleri güncellenemedi:",
          updateProductsError.message
        );
      }
    }
    ...
    return {
      ...
      message: `Kategori silindi. ${
        options.deleteProducts
          ? "Ürünler de silindi."
          : "Ürünlerin kategori bağlantısı kaldırıldı."
      }`,
    };
  } catch (error: any) {
    return { success: false, message: error.message };
...
// 5. Çevirileri Kaydet
export async function saveCategoryLocalizationsAction(
  categoryId: number,
  localizations: any[]
) {
  try {
...
// 6. Kategori Parent'ını Güncelle (Drag & Drop için)
export async function updateCategoryParentAction(
  categoryId: number,
  ...
  try {
    ...
    // Derinlik kontrolü (maksimum 3 seviye)
    if (newParentId) {
      // Yeni parent'ın derinliğini kontrol et
      const { data: parentCategory, error: parentError } = await adminSupabase
        .from("categories")
        .select("parent_id")
        .eq("id", newParentId)
        .single();
      ...
      if (parentError)
        throw new Error("Parent kategori bulunamadı: " + parentError.message);

      // Eğer parent'ın da bir parent'ı varsa, bu 3. seviye olur
      if (parentCategory?.parent_id) {
        throw new Error("Kategori maksimum 3 seviye derinliğe sahip olabilir.");
      }
    }
    ...
    revalidatePath("/admin/categories");
    return { success: true, message: "Kategori konumu güncellendi." };
  } catch (error: any) {
    return { success: false, message: error.message };
  }
}
