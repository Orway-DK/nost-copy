...
export default async function CategoryDetailPage(props: PageProps) {
    ...
    const catQuery = !isNew && categoryId
        ? adminSupabase.from("categories").select("*").eq("id", categoryId).single()
        : Promise.resolve({ data: null });

    const locQuery = !isNew && categoryId
        ? adminSupabase.from("category_translations").select("*").eq("category_id", categoryId)
        : Promise.resolve({ data: [] });

    const [allCatsRes, catRes, locRes] = await Promise.all([allCatsQuery, catQuery, locQuery]);
