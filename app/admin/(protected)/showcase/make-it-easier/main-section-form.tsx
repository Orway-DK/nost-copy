...
import { translateTextAction } from "@/app/admin/actions"; // Çeviri aksiyonu
import { IoSave, IoLanguage, IoSparkles } from "react-icons/io5"; // Sparkles ikonu eklendi
...
export default function MainSectionForm({ initialData }: { initialData: any }) {
    ...
    // Form State Hazırlığı
    const [form, setForm] = useState(() => {
        const translations = LANGS.map(l => {
            const found = initialData?.make_it_easier_translations?.find((t: any) => t.lang_code === l);
            return {
                lang_code: l,
                id: found?.id,
                title: found?.title || "",
    ...
    const handleAutoTranslate = async () => {
        ...
        try {
            ...
            // Çevrilecek alanlar listesi
            const textFields = [
                'title', 'titletext', 
                'tip1_title', 'tip1_text', 
                'tip2_title', 'tip2_text', 
                'tip3_title', 'tip3_text'
            ];

            // Diğer dilleri döngüye al (Promise.all ile paralel yapalım)
            const tasks = newTranslations.map(async (target, index) => {
                if (target.lang_code === activeLang) return; // Kaynak dili atla

                // Her bir alan için çeviri isteği
                ...
                // İkonları da kopyala (Çeviriye gerek yok, aynısı olsun)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-20">
            ...
            <div className="lg:col-span-1 space-y-4">
                <div className="card-admin p-4">
                    ...
                    <div className="relative aspect-square w-full rounded-lg overflow-hidden border flex items-center justify-center bg-[var(--admin-input-bg)]"
                         style={{ borderColor: "var(--admin-input-border)" }}>
                        {imgUrl ? (
                            <Image src={imgUrl} alt="preview" fill className="object-contain p-2" unoptimized />
                        ) : (
                            <span className="text-xs opacity-50">Görsel Yok</span>
                        )}
                        <button 
                    ...
                    <input className="admin-input mt-2 text-xs" value={form.image_link} readOnly />
                </div>
            </div>
                ...
                <div className="card-admin p-6 space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea className="admin-textarea h-24" value={currentTrans.titletext} onChange={e => updateTrans("titletext", e.target.value)} />
                        </div>
                    </div>

                    {/* MADDELER */}
                    <div className="grid gap-4 pt-4 border-t" style={{ borderColor: "var(--admin-card-border)" }}>
                        <h4 className="font-semibold text-sm" style={{ color: "var(--admin-fg)" }}>3 Adım (İpuçları)</h4>
                        
                        {[1, 2, 3].map(num => (
                            <div key={num} className="p-4 rounded border grid grid-cols-1 sm:grid-cols-12 gap-4 items-center"
                                 style={{ backgroundColor: "var(--admin-input-bg)", borderColor: "var(--admin-input-border)" }}>
                                <div className="sm:col-span-1 text-center font-bold opacity-50">{num}</div>
                                <div className="sm:col-span-3">
                                    <label className="text-xs opacity-60 block mb-1">İkon (Iconify)</label>
                                    <input className="admin-input text-xs font-mono" 
                                        // @ts-ignore
                                        value={currentTrans[`tip${num}_icon`]} onChange={e => updateTrans(`tip${num}_icon`, e.target.value)} 
                                    />
                                </div>
                                <div className="sm:col-span-4">
                                    <label className="text-xs opacity-60 block mb-1">Başlık</label>
                                    <input className="admin-input text-sm" 
                                        // @ts-ignore
                                        value={currentTrans[`tip${num}_title`]} onChange={e => updateTrans(`tip${num}_title`, e.target.value)} 
                                    />
                                </div>
                                <div className="sm:col-span-4">
                                    <label className="text-xs opacity-60 block mb-1">Açıklama</label>
                                    <input className="admin-input text-sm" 
                                        // @ts-ignore
                                        value={currentTrans[`tip${num}_text`]} onChange={e => updateTrans(`tip${num}_text`, e.target.value)} 
                                    />
                                </div>
                            </div>
                        ))}
