// C:\Projeler\nost-copy\app\admin\(protected)\showcase\landing\_components\HighlightsManager.tsx
'use client'
...
export default function HighlightsManager ({
  ...
  const handleEdit = (item: any) => {
    const transMap: any = {}
    item.landing_highlight_translations.forEach(
  ...
  if (editing) {
    ...
    return (
      <div className='card-admin p-6 space-y-6 animate-in fade-in slide-in-from-bottom-4 shadow-lg'>
        ...
        >
          <div
            className='flex gap-2 border-b mb-4 overflow-x-auto pb-1'
            style={{ borderColor: 'var(--admin-card-border)' }}
          >
            {LANGS.map(l => (
              <button
                ...
                className={`px-4 py-2 text-sm font-bold transition-colors border-b-2 whitespace-nowrap ${
                  activeLang === l
        ...
        <div
          className='flex justify-end pt-4 gap-3 border-t'
          style={{ borderColor: 'var(--admin-card-border)' }}
        >
          <button
            onClick={() => setEditing(null)}
    ...
    <div className='space-y-6'>
      ...
      <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-3'>
        {initialItems.map(item => (
          <div
            key={item.id}
            className='card-admin p-4 flex items-center gap-4 hover:shadow-md transition-all hover:-translate-y-1'
          >
            <div
              className='p-3 rounded-lg text-2xl font-bold flex items-center justify-center w-12 h-12 flex-shrink-0'
              style={{
            ...
            <div className='flex-1 min-w-0'>
              <div
                ...
                style={{ color: 'var(--admin-fg)' }}
              >
                {item.landing_highlight_translations?.find(
                  (t: any) => t.lang_code === 'tr'
                )?.text || '-'}
              </div>
              <div
                className='text-xs font-mono truncate opacity-60'
                style={{ color: 'var(--admin-muted)' }}
              >
                {item.icon}
              </div>
            ...
            <div className='flex flex-col gap-2'>
              <button
                ...
                className='text-[var(--admin-muted)] hover:text-[var(--admin-accent)] hover:bg-[var(--admin-input-bg)] p-1.5 rounded transition-colors'
              >
                <IoPencil />
              </button>
              <button
                onClick={() => handleDelete(item.id)}
                className='text-[var(--admin-muted)] hover:text-[var(--admin-danger)] hover:bg-[var(--admin-input-bg)] p-1.5 rounded transition-colors'
              >
                <IoTrash />
              </button>
            </div>
