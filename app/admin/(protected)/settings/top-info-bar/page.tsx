...
export default function AnnouncementBarSettings() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Duyuru bandı güncellendi!", { id: toastId });
    } catch (error: any) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      <div className="admin-page-header">
        <div>
          <h1 className="admin-page-title">Duyuru Bandı (Top Bar)</h1>
          <p className="text-[var(--admin-muted)] text-sm">
            Sitenin en tepesindeki ince promosyon şeridini yönetin.
          </p>
        </div>
      </div>
      ...
      >
        <h3 className="text-lg font-bold mb-4 flex items-center gap-2 text-[var(--admin-fg)]">
          <FaBullhorn className="text-[var(--admin-warning)]" /> İçerik (
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              placeholder="Örn: Tüm kartvizitlerde %20 indirim!"
            />
          </div>
      ...
      <div className="flex justify-end pt-4">
        <button
          onClick={handleSave}
          className="btn-admin btn-admin-primary px-8 py-2.5 gap-2"
        >
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-700">
        <p className="font-bold mb-1">ℹ️ Bilgi:</p>
        <p>
          Telefon ve Adres bilgileri artık <strong>Lokasyon Yönetimi</strong>{" "}
          sayfasında, "Varsayılan (HQ)" olarak seçilen ofisten otomatik
          çekilmektedir.
        </p>
      </div>
    </div>
