...
export async function upsertServiceAction (data: any) {
  try {
    ...
    if (!slug && data.translations) {
      const trTitle = data.translations.find(
        (t: any) => t.lang_code === 'tr'
      )?.title
      const enTitle = data.translations.find(
...
// app/admin/(protected)/services/actions.ts dosyasına ekle:

// 3. ID'ye Göre Tek Hizmet Getir (Düzenleme Sayfası İçin)
export async function getServiceByIdAction (id: number) {
  try {
    await checkAuth()
    ...
    // Form'un beklediği { translations: { tr: {...}, en: {...} } } formatına çeviriyoruz
    const formattedData = {
      ...data,
      translations: data.service_translations.reduce((acc: any, curr: any) => {
        acc[curr.lang_code] = {
          id: curr.id, // ID önemli, güncelleme yaparken lazım olacak
          title: curr.title || '',
